...
class Customizer {
			...
			function () {
				...
				<script data-name="colibri-preview-options">
					...
					var colibri_ADDITIONAL_JS_DATA =
							<?php
							echo wp_json_encode(
								(object) Hooks::colibri_apply_filters(
	...
	public function registerSettings( $wp_customize ) {
		...
		$this->settings->walkFirstLevel(
			function ( $id, $data ) use ( $wp_customize ) {
		...
		wp_localize_script(
			...
			Hooks::colibri_apply_filters(
				...
				array(
					...
					'colibri_autofocus_aliases' => (object) Hooks::colibri_apply_filters(
