...
function colibriwp_post_categories( $attrs = array() ) {
	...
	if ( $categories ) {
		foreach ( $categories as $category ) {
			$html .= sprintf(
				'<a class="d-inline-block" href="%1$s">%2$s</a>',
...
function colibriwp_theme_print_footer_copyright() {
	...
	<div class="h-global-transition-all">
		...
		<?php
		printf(
			__( 'Built using WordPress and %s', 'colibri-wp' ),
			'<a target="_blank" href="https://lucasbonomo.com/">Hero Theme</a>'
		);
		?>
		 .
	</div>
